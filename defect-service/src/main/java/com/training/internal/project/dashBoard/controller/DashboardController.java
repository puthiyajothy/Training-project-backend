package com.training.internal.project.dashBoard.controller;
...
import com.training.internal.project.dashBoard.service.DashboardService;
import com.training.internal.project.dashBoard.services.impl.DashboardServiceImpl;
...
public class DashboardController {
	...
	public ResponseEntity<Float> getTotalCount() {
		return new ResponseEntity<>(dashboardServiceimp.CalculateLow(count, reject, low, rejectlow), HttpStatus.OK);

	}

	@GetMapping("/gethightcount")
	public ResponseEntity<Float> getTotalCounthighseverity() {
		return new ResponseEntity<>(dashboardServiceimp.Calculatseverityhigh(count, reject, High, rejectHigh),
				HttpStatus.OK);
	...
	public ResponseEntity<Float> getTotalCountmedium() {
		return new ResponseEntity<>(dashboardServiceimp.CalculateMedium(count, rejectlow, medium, rejectmedium),
				HttpStatus.OK);
	...
	public ResponseEntity<Double> getSeverityIndex() {
		return new ResponseEntity<>(dashboardServiceimp.calculateSeverityIndex(), HttpStatus.OK);

	}
	
	@GetMapping("/totalcount")
	public ResponseEntity<Long> Totaldefect() {
		return new ResponseEntity<>(dashboardServiceimp.TotalCount(), HttpStatus.OK);
...
//	<--Total Severity Low Defect Count Controller-->
	...
	public ResponseEntity<Integer> countseveritytotalLow() {
		try {
			...
			return new ResponseEntity<>(dashboardServiceimp.countseveritytotalLow(), HttpStatus.OK);
...
//	<--Total Severity Medium Defect Count Controller-->
	...
	public ResponseEntity<Integer> countseveritytotalmedium() {
		try {
			logger.info("Dashboard Controller--> successfully get Total Severity Low ");
			return new ResponseEntity<>(dashboardServiceimp.countseveritytotalmedium(), HttpStatus.OK);
...
//	<--Total Severity High Defect Count Controller-->
	...
	public ResponseEntity<Integer> countseveritytotalhigh() {
		try {
			...
			return new ResponseEntity<>(dashboardServiceimp.countseveritytotalhig(), HttpStatus.OK);
//					defectRepo.countBySeverityMedium(), HttpStatus.OK);
//			defectRepo.countBySeverity(),HttpStatus.OK);
		} catch (Exception e) {
			logger.error("Dashboard Controller--> Error" + e.getMessage());
